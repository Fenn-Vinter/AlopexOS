...
[[nodiscard]] fn chopPath(AlopexOS::Path path) -> AlopexOS::Expected<dynarr<AlopexOS::string>> {
...
fn AlopexOS::AbtrFS::write_file(AlopexOS::Path path, dynarr<byte> data) -> AlopexOS::errorCode {
    ...
    for (u64 b = 0; b < data_blocks_count; b++) {
        ...
        if (!write_block(file_header.payload_offset + b, block_buffer.data())) {
            return AlopexOS::errorCode::WriteFailed;
        }
    }
